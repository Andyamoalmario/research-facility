...
export function Inspector() {
  ...
  const roomCount = useBuildingStore((s) => s.rooms.length);
  ...
  return (
    <aside className="flex h-full w-64 flex-col gap-3 border-l border-blue-night bg-bg-panel p-3">
      ...
      <div className="mt-auto flex flex-col gap-1 font-mono text-xs text-text-secondary">
        <div>
          Muri costruiti: <span className="text-text-primary">{wallCount}</span>
        </div>
        <div>
          Stanze rilevate: <span className="text-green-monitor">{roomCount}</span>
        </div>
      </div>
    </aside>
  );
